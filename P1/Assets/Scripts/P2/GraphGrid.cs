...
namespace UCM.IAV.Navegacion
{
    ...
    public class GraphGrid : Graph {
        ...
        {
            return vertexObjs[x, y].GetComponent<Vertex>();
        }

        //return final position to get to
        public Vector2 FinalPos()
        {
            return fin;
        }

        //return position from the minotaur
        public Vector2 GetEnemyPos()
        {
            Vector2 v = new Vector2(minotauro.transform.position.x, minotauro.transform.position.z);
            return v;
        }
        ...
        public override void Load()
        {
            ...
            for (int i = 0; i < numCols; i++) {
                for (int j = 0; j < numRows; j++) {
                    if (mapVertices[i, j]) {
                        vertexObjs[i,j] = Instantiate(vertexPrefab, new Vector3(i, 0, j), Quaternion.identity, transform);
                        vertexObjs[i,j].GetComponent<Vertex>().id = id;
                        id++;
                    }
                    else
                    {
            ...
            //Generamos las conexiones del grafo
            for (int i = 0; i < numCols; i++)
            {
                for (int j = 0; j < numRows; j++)
                {
                    if(mapVertices[i,j])
                    {
                        if (j + 1 < numCols && mapVertices[i, j + 1])
                        {
                            Edge e = new Edge(vertexObjs[i, j + 1].GetComponent<Vertex>());
                            vertexObjs[i, j].GetComponent<Vertex>().neighbours.Add(e);
                        }
                        if (j - 1 >= 0 && mapVertices[i, j - 1])
                        {
                            Edge e = new Edge(vertexObjs[i, j - 1].GetComponent<Vertex>());
                            vertexObjs[i, j].GetComponent<Vertex>().neighbours.Add(e);
                        }
                        if (i + 1 < numRows && mapVertices[i + 1, j])
                        {
                            Edge e = new Edge(vertexObjs[i + 1, j].GetComponent<Vertex>());
                            vertexObjs[i, j].GetComponent<Vertex>().neighbours.Add(e);
                        }
                        if (i - 1 >= 0 && mapVertices[i - 1, j])
                        {
                            Edge e = new Edge(vertexObjs[i - 1, j].GetComponent<Vertex>());
                            vertexObjs[i, j].GetComponent<Vertex>().neighbours.Add(e);
                        }
        ...
        public void GenerateMap()
        {
            ...
            switch (x)
            {
                case 0: // Up ini
                    x = 0;
                    y = UnityEngine.Random.Range(1, nC);
                ...
                case 1: // Down ini
                    ...
                    break;
                case 2: // Left ini
                    x = UnityEngine.Random.Range(1, nR);
                    y = 0;
